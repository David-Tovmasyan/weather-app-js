...
let cityName=document.querySelector("main .wrapper .city") // html tag
let temperature=document.querySelector("main .wrapper .temperature");
...
async function getCurrentData(city){
    ...
    let data=await res.json();
    setCurrentData([data.main, data.weather[0].main, data.name, countryListAlpha2[data.sys.country] ]) // sending required data
}

function setCurrentData(data){
    temperature.innerHTML=`${Math.round(data[0].temp)}<span>&#8451</span>` //Celsiusi nshan
    cityName.innerHTML=`${data[2]}, ${data[3]}` 
    if(data[2]==data[3]){   //erkri anun@ grelu depqum
...
function setHourlyData(data){
    for(let i=0;i<boxNumber;i++){
        timeBox[i].innerHTML=`<div>${getOnlyTime(data[i+1].dt_txt)}</div>`;
    }
    for(let i=0;i<boxNumber;i++){  // 5 zam exanak
        hoursBox[i].innerHTML=`<div>${parseInt(data[i+1].main.temp)}<span>&#8451;</span></div>`; //+ 1 vor hajord zamic sksi
    }
}
